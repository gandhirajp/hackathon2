...
const Questionnaire = ({ careerPaths }) => {
  ...
  const generateCareerSuggestions = () => {
    ...
    Object.keys(answers).forEach((questionKey) => {
      const question = careerPaths.find((q) => q.question === parseInt(questionKey));
      if (question) {
        const selectedChoice = question.choices.find((choice) => choice.value === answers[questionKey]);
        if (selectedChoice) {
  ...
  return (
    <div className="questionnaire-container">
      <h1 className="questionnaire-title">Let's find you a suitable career path</h1>
      <p className="questionnaire-title">Help us with a few simple questions to know you better</p>
      {openPopup && <Popup title='Congratulations!' content={`Based on your choices, your career paths are:`} highlight={openPopup} />}
              ...
              {question.choices.map((choice) => (
                <div key={choice.value} className="choice">
                  <input
                    ...
                    disabled
                  />
                  <label htmlFor={`${question.question}-${choice.value}`} className="choice-label">
                    {choice.label}
                  </label>
                </div>
              ))}
